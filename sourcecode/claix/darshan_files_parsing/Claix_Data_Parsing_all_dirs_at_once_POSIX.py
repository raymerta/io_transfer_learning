...
os.environ["LD_LIBRARY_PATH"]="~/Software/darshan-3.3.1/lib:$LD_LIBRARY_PATH"
...
def read_processed_files_from_log(log_cursor: sqlite3.Cursor):
    files_from_log_db = log_cursor.execute(
        "SELECT File from log;"  # WHERE Processed = TRUE or (Processed = FALSE and Comments IS NULL)
    ).fetchall()
