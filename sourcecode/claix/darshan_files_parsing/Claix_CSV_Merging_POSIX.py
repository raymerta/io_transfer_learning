...
def find_all_files_recursively(dataset_dir: Path):
    ...
    # If given the dir that directly contains the Darshan files to parse
    if not len(dirs_to_process):
        csv_files_to_process = find_files_in_current_dir(dataset_dir)
        return csv_files_to_process

    # If Darshan files are stored in the subdirs of the dataset dir
...
if __name__ == "__main__":
    ...
    combined_df.to_csv(dataset_dir.joinpath(out_file_name), index=False)
